...
class RouterFTPHandler(FTPHandler):
    ...
    def on_file_received(self, file_path):
        ...
        # Router expects a 5-part header: CHUNK:chunk_number:chunk_size:num_chunks:target_node:sender_node
        ...
        if target_node and not any(node_info['node_name'] == target_node for node_info in self.server.manager.network.ip_map.values()):
        ...
        if not any(node_info['node_name'] == sender_node for node_info in self.server.manager.network.ip_map.values()):
            logger.error(f"Invalid sender node '{sender_node}' in header from {file_path}. Node does not exist.")
            try:
                os.remove(file_path)
            except OSError:
                pass
        ...
        with self.server.manager.active_nodes_lock:
            if target_node and target_node not in self.server.manager.active_nodes:
        ...
        if chunk_number == 1:
            ...
            with open(self.session_state["temp_file_path"], 'wb') as f:
                f.write(payload)
            with self.server.manager.pending_files_lock:
                self.server.manager.pending_files[folder_name] = (target_node, original_filename, sender_node)
        else:
            # Add validation for subsequent chunks
            if original_filename != self.session_state.get("current_filename") or \
                target_node != self.session_state.get("target_node") or \
                sender_node != self.session_state.get("sender_node"):
                logger.error(f"Chunk {chunk_number} for {original_filename}:{target_node}:{sender_node} does not match expected {self.session_state.get('current_filename')}:{self.session_state.get('target_node')}:{self.session_state.get('sender_node')}")
        ...
        logger.info(f"Received chunk {chunk_number}/{self.session_state['expected_chunks']} for {original_filename} (folder: {self.session_state['folder_name']}, target: {target_node}, sender: {sender_node})")
        ...
        if self.session_state["received_chunks"] == self.session_state["expected_chunks"]:
            ...
            logger.info(f"Stored {original_filename} in folder {self.session_state['folder_name']} for {target_node} from {sender_node}")
            self.respond(f"226 Transfer complete: {original_filename} stored for {target_node}")
            
            self.server.manager.check_node_and_forward(self.session_state["folder_name"], target_node, final_path, original_filename, sender_node)
